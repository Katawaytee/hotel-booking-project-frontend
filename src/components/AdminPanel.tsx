'use client'
import addHotel from "@/libs/addHotel"
...
export default function AdminPanel({token} : {token:string}) {
  ...
  async function toAddHotel(addHotelForm:FormData) {
    ...
    await addHotel(HotelInfo,token);
  ...
  return(
    <form action={toAddHotel} className="w-full bg-slate-100 m-5 p-5 flex flex-col items-center">
